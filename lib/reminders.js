...
module.exports = function startReminders (send) {
  db.db.users.find({}, function (err, docs) { // Add all users with current reminders
    if (err) console.err('Error in start reminders: ' + err);
    docs.forEach(function (doc) {
      console.log('[db] user: %s, reminder: %d', doc.print_name, doc.reminder);
      if (doc.reminder) {
